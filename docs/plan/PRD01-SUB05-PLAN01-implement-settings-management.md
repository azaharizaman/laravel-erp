...
|------|-------------|-----------|------|
| TASK-001 | Create package directory structure: `packages/settings-management/` with subdirectories `src/`, `database/migrations/`, `database/seeders/`, `config/`, `tests/`. Initialize `composer.json` with package name `azaharizaman/erp-settings-management`, namespace `Azaharizaman\Erp\SettingsManagement`, require Laravel 12+, PHP 8.2+. | | |
