...
|------|-------------|-----------|------|
| TASK-001 | Create package directory structure: `packages/audit-logging/` with subdirectories `src/`, `database/migrations/`, `database/seeders/`, `config/`, `tests/`. Initialize `composer.json` with package name `azaharizaman/erp-audit-logging`, namespace `Azaharizaman\Erp\AuditLogging`, require Laravel 12+, PHP 8.2+, Spatie Laravel Activitylog as base dependency. | | |
